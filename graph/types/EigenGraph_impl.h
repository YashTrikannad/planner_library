...
{
    {
        ...
        {
            neighbors[0] =  {node.row_index_ -1, node.column_index_ };
            neighbors[1] =  {node.row_index_, node.column_index_ + 1};
            neighbors[2] =  {node.row_index_ + 1, node.column_index_ };
            neighbors[3] =  {node.row_index_ , node.column_index_ - 1};
        ...
        {
            neighbors[index++]  = {node.row_index_ - 1, node.column_index_};
        }
        if(node.column_index_ != cols_)
        {
            neighbors[index++]  = {node.row_index_, node.column_index_+1};
        }
