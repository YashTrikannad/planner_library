...
int main()
{
    ...
    // Generate a Map
    map_generator.generate_map(1000, 1000, 20);

    // Test GridMaps
    auto map = map_generator.get_map();

    auto vector_graph = pl::graph::cost_graph<pl::graph::vector_graph, std::vector<std::vector<size_t>>>{map};

    // Set Up Planner
    pl::algorithms::astar<pl::graph::cost_graph<pl::graph::vector_graph, std::vector<std::vector<size_t>>>,
            std::vector<pl::common::NodeIndex2d>, pl::common::NodeIndex2d>
            eigen_planner(&vector_graph);

    // Find Path
    eigen_planner.find_path<8>({0, 0}, {750, 500});
