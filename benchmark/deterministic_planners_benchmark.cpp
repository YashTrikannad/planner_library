...
static void BM_VectorGraphCreation(benchmark::State& state) {
    ...
    for (auto _ : state)
    {
        ...
        map_generator.generate_map(state.range(0), state.range(1), state.range(2));
        auto map = map_generator.get_map();
        auto vector_graph = pl::graph::graph<std::vector<std::vector<size_t >>, size_t >(map);
    }
}
...
static void BM_EigenGraphConversion(benchmark::State& state) {
    std::set<int> data;
    for (auto _ : state)
    {
        pl::map::ConstraintMapGenerator<size_t, 0, 1> map_generator;
...
BENCHMARK(BM_EigenGraphConversion)
        ->Args({100, 100, 5})
        ->Args({200, 200, 5})
        ->Args({400, 400, 5})
        ->Args({800, 800, 5})
...
static void BM_EigenGraphCreation(benchmark::State& state) {
    std::set<int> data;
    for (auto _ : state)
    {
        pl::map::ConstraintMapGenerator<size_t, 0, 1> map_generator;
        ...
        auto map = map_generator.get_map();
        auto EigenMap = pl::common::convert_2dvector_to_eigen(map);
        auto vector_graph = pl::graph::graph<Eigen::MatrixXd, double>(EigenMap);
    }
}
BENCHMARK(BM_EigenGraphCreation)
        ->Args({100, 100, 5})
        ->Args({200, 200, 5})
        ->Args({400, 400, 5})
        ->Args({800, 800, 5})
...
static void BM_CostVectorGraphCreation(benchmark::State& state) {
    std::set<int> data;
    for (auto _ : state)
    {
        pl::map::ConstraintMapGenerator<size_t, 0, 1> map_generator;
...
BENCHMARK(BM_CostVectorGraphCreation)
        ->Args({100, 100, 5})
        ->Args({200, 200, 5})
        ->Args({400, 400, 5})
        ->Args({800, 800, 5})
...
static void BM_VectorCostGraphAstarSearch(benchmark::State& state) {
    std::set<int> data;
    for (auto _ : state)
    {
        pl::map::ConstraintMapGenerator<size_t, 0, 1> map_generator;
        ...
        eigen_planner.find_path<8>({0, 0},
                {static_cast<size_t>(state.range(0)/2), static_cast<size_t>(state.range(1)/2)});
        const auto eigen_path = eigen_planner.get_path();
    }
}
BENCHMARK(BM_VectorCostGraphAstarSearch)
        ->Args({100, 100, 5})
        ->Args({200, 200, 5})
        ->Args({400, 400, 5})
        ->Args({800, 800, 5})
